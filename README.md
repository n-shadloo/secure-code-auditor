...
| 3 Sep 2026 | v1.53.0 address check, header limits, and rotation unit | Three verified gaps close, each proved by execution rather than by reading. `a01-broken-access-control.md` gains the mechanical form of the post-resolution address check that its SSRF section already required but never spelled: `not ip.is_global or ip.is_multicast`. The predicate refuses loopback, every private range, the link-local range that carries `169.254.169.254`, the shared address space `100.64.0.0/10`, and each IPv4-mapped IPv6 form of those, and the multicast half is needed because `is_global` is `True` for a multicast address. The floor is Python 3.12.4, established by running the classification on both sides of every boundary from 3.9 to 3.14: 3.12.3 reports `2002:a9fe:a9fe::`, the 6to4 form of the metadata address, as global, and 3.12.4 refuses it, with the same fix in 3.9.20, 3.10.15, and 3.11.10 -- the CPython documentation attributes the change to 3.13 alone, which is the feature release rather than the first release that carries it. The predicate is not a complete refusal list and the section does not claim one: `64:ff9b::/96` stays global on every release through 3.14.0, so behind a NAT64 gateway `64:ff9b::a00:1` still reaches `10.0.0.1`, which is why the allowlist keeps the lead and the named ranges stay. `deployment-and-runtime.md`: the three request-size limits sat in an unlabelled bullet directly under the bullet that describes uvicorn and Daphne, so a reader on ASGI could take them as theirs. They are Gunicorn options -- `--limit-request-line` 4094, `--limit-request-fields` 100, and `--limit-request-field-size` 8190 on Gunicorn 26.2.0 -- and uvicorn has none of them. Its counterpart is `--h11-max-incomplete-event-size`, whose 16384 default comes from `h11.Connection` at h11 0.16.0, and only `h11_impl.py` reads it; the `httptools_impl.py` and `zttp_impl.py` of uvicorn 0.52.4 do not, and `--http auto` resolves to `HttpToolsProtocol` wherever httptools 0.8.0 is installed, which is what `uvicorn[standard]` brings. Under that combination the reverse proxy is the only header bound. `service-identity-and-secrets.md`: the `SECRET_KEY_FALLBACKS` subsystem table is re-verified at Django 6.1, which PyPI records as released on 5 Aug 2026, and the annotation now names 6.1, 6.0, and 5.2. Every row was executed at 6.1, 6.0.8, and 5.2.17 and none moved: `Signer.fallback_keys` defaults to the setting, `AbstractBaseUser.get_session_auth_fallback_hash` still drives the `cycle_key()` and the re-stamp that upgrades a session in place, `PasswordResetTokenGenerator.secret_fallbacks` resolves, an old-key signed cookie and an old-key `signing.dumps` value both verify under the new key, and `SECRET_KEY` appears nowhere in `django/middleware/csrf.py`, so the CSRF row holds. The rotation procedure gains its unit: the rolling-fleet split counted instances of one deploy, so a worker fleet, a scheduled job, or a second service on the same key fell outside it, and a compromise rotation that deliberately skips the fallback does not cut while such a fleet still holds the leaked key. No claim, control, or example is removed. The library-index date does not move. |
