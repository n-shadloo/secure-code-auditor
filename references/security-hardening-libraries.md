...
**How to read a floor here.** A floor binds the project's own requirements,
and not the integration that installs the distribution. An integration can
declare a floor below the advisory, so the install misses the fix.
`python-ldap` and `oauthlib` below are the two recorded cases. Pin the
transitive distribution in the project yourself.

A row that states no floor is not a row whose floor is clear. It records that
nobody wrote one. Check the installed version against the advisory sources
before you quote the row. Check it also where a scanner is quiet and the
distribution wraps a native upstream project. An advisory there can exist only
against the upstream repository, with no entry for the Python distribution,
and no Python scanner raises it. `grpcio` below is that case, and currency is
the control for it.
...
| JWT | SimpleJWT `5.5.1` (21 Jul 2025; re-checked 9 Aug 2026) | **Conditional** for supported Django 5.2 projects; not advertised for Django 6 — the published classifiers stop at Django 5.2, confirmed on re-check. Two releases in the eighteen months to that date is a slowing cadence rather than an advisory, and none is outstanding. Minimum `5.5.1` includes the CVE-2024-22513 fix. Configure algorithm/issuer/audience/lifetimes/rotation/denylisting. Two defaults decide the rest. `SIGNING_KEY` defaults to Django's `SECRET_KEY`, so a `SECRET_KEY` leak becomes API token forgery, and a `SECRET_KEY` rotation invalidates every issued token at once. Set an independent key, exactly as the `django-ninja-jwt` row below requires. Denylisting also stays inert until `rest_framework_simplejwt.token_blacklist` is in `INSTALLED_APPS` and its migrations are applied. The settings alone revoke nothing. Prove the revocation path with a test that rejects a denylisted refresh token. |
...
| Dependency advisories | `pip-audit==2.10.1` | **Recommend as one input.** PyPA-maintained, Apache-2.0; Python >=3.10. Its sources are the PyPI advisory service and OSV rather than NVD-derived CPE matching, which is the reason to keep it primary for Python after NVD moved to risk-based triage in April 2026 — see `a03-software-supply-chain.md`, "SBOM, scan gate, and provenance". It does not prove maintenance, provenance, compatibility, configuration, or safety. |
| CycloneDX SBOM generation | `cyclonedx-bom==7.3.1` (23 Jul 2026) | **Conditional: recommend only where a committed CycloneDX SBOM is a stated requirement.** Apache-2.0; OWASP CycloneDX project; Python >=3.9 through 3.14. The gate's first question decides most cases against it — `pip-audit` already emits CycloneDX with `-f cyclonedx-json`, so a project that runs it in CI has the artifact without a second dependency, and this entry is for the case where a standalone SBOM from the lockfile is what is being asked for. Two facts belong in the pin. The distribution is `cyclonedx-bom` and the command is `cyclonedx-py`; a distribution named `cyclonedx-py` also exists on PyPI and is an alias package sitting at 1.0.1, so requiring the command name gets the wrong thing. And it records no component hashes — generating from a `pip-compile --generate-hashes` file emits components with no `hashes` member, verified in both `requirements` and `environment` modes on 9 Aug 2026 — so it is inventory, never integrity evidence. Pass `--spec-version` explicitly: 1.7 has been published since October 2025 and 7.3.1 still defaults to 1.6. Exit plan is clean; the output is a standard artifact and Syft or pip-audit's emitter produces it too. |
...
| LDAP directory queries and authentication | `django-auth-ldap==5.3.0` (26 Dec 2025) over `python-ldap>=3.4.5`, latest `3.4.7` (20 May 2026); escaping behavior and advisory checked 8 Aug 2026 | **Recommend the integration where a directory is the identity source; require the `python-ldap>=3.4.5` floor explicitly.** There is no built-in alternative — a directory client is a dependency by definition — so the gate here is about the floor and the defaults rather than the choice. django-auth-ldap is BSD, Python >=3.10, Production/Stable, and declares Django 4.2, 5.2, and 6.0; it escapes filter arguments through `ldap.filter.escape_filter_chars` with escaping on by default, which is the reason to prefer it over hand-assembled filter strings. Advisory: `python-ldap` before 3.4.5 could be made to skip escaping when a `list` or `dict` reached `escape_filter_chars` under the non-default `escape_mode=1` (CVE-2025-61911, moderate); 3.4.5 type-checks the argument. django-auth-ldap's own requirement is only `python-ldap>=3.1`, so installing the integration does not deliver the fix — pin `python-ldap` yourself. Operational cost: `python-ldap` builds against the OpenLDAP client libraries, so it needs system headers in the build image rather than a wheel alone. Review the search filters for any place the project still formats its own, and confirm group-to-permission mapping separately — a successful bind is authentication, not authorization. One more default belongs in the same check: `AUTH_LDAP_PERMIT_EMPTY_PASSWORD` is `False`, and it must stay `False`. A directory server answers a simple bind that carries an empty password as an anonymous bind, and an anonymous bind succeeds, so that setting turns a blank password into a successful login for any username. Exit plan: filter escaping is one function call, so a different client keeps the same call-site discipline. See `a05-injection.md`, "Directory and LDAP injection". |
...
| `bleach==6.4.0` (5 Jun 2026; checked 20 Aug 2026) | **Reject for new use; existing-install audit only.** The 6.4.0 release carries the `Development Status :: 7 - Inactive` classifier, and the `mozilla/bleach` repository is archived with its last push on the same day, so no CVE response is ahead of it. The cadence had already fallen to roughly one release a year before that. The successor for HTML sanitization is the `nh3` entry above — bindings to the Rust `ammonia` crate, allowlist-based, and still releasing, most recently on 22 Jun 2026. Migrate `bleach.clean()` calls to `nh3.clean()` and re-verify every allowlist argument, because the defaults differ. The control itself is in `a05-injection.md`, "Template injection and server-side output". Dated claim: re-check both packages on PyPI at review time. |
...
| Concern | Choice and version | Disposition and review notes |
|---|---|---|
| Object permissions (ACL rows) | `django-guardian==3.3.3` (22 Jul 2026) | **Conditional on a Django 6.0 baseline; recommend on 5.2.** BSD-2-Clause; Python >=3.10; actively released (3.3.0–3.3.3 all in 2026), but classifiers still stop at Django 5.2 — do not infer 6.0 support. Requires adding `guardian.backends.ObjectPermissionBackend` to `AUTHENTICATION_BACKENDS`; without it `has_perm(perm, obj)` stays a no-op. Review anonymous-user handling first: guardian creates a real anonymous user row, and that row is a live grant target. An object permission assigned to it passes `has_perm(perm, obj)` for every unauthenticated request. It renders in the admin like a placeholder account, so the grant looks inert and is not. Deny object permissions to that row by policy, assert that in a test, and rate a grant found on it as a finding on its own weight. Also review `get_objects_for_user()` query cost, and generic- vs direct-FK permission models on hot paths. |
...
| Impersonation | `django-hijack==3.7.8` (19 Apr 2026) | **Recommend.** MIT; Python >=3.10; Django 6.0 classifier present; v3 is a security-focused rewrite. Keep `HIJACK_PERMISSION_CHECK` at `superusers_only` and keep acquisition POST-only with CSRF. It does not time-box, re-authenticate, or scope down the session — add those yourself. |
...
| Policy library | `oso==0.27.3` (13 Jan 2024) | **Reject for new use.** The open-source library was deprecated on 18 Dec 2023 in favor of the hosted Oso Cloud; `django-oso` is part of the same deprecated line. Existing installs should plan migration. |
...
Dynamic client registration is a capability rather than a control, and an open
registration endpoint fails open. Any caller then creates an OAuth client
against your authorization server. Keep registration off unless a named
consumer needs it. Where it is on, require an initial access token on the
registration endpoint, and review the scopes an auto-approved client asks
for.
...
| Schema-per-tenant | `django-tenants==3.14.0` (5 Aug 2026) | **Conditional, and pin it exactly.** MIT; Production/Stable; declares Django 5.2, 6.0, and 6.1 and Python 3.10–3.13, which makes it one of the few entries here already carrying the new line — it dropped the 4.2 classifier in the same move. Adopt only where schema-per-tenant genuinely is the architecture: it requires session-mode pooling because `search_path` is session state, and migration time scales linearly with tenant count. It also selects the tenant from the request host, which makes `ALLOWED_HOSTS` the tenant allowlist and not only the Host-poisoning control that `a02-security-misconfiguration.md`, "DEBUG and ALLOWED_HOSTS" audits it as. List the tenant domains exactly. Leave `USE_X_FORWARDED_HOST` off unless the proxy overwrites that header on every request, because it moves tenant selection onto a header the client sets. It is not the default answer to multi-tenancy — scoped querysets plus a cross-tenant test suite are. The pin is not a formality: it went 3.10.2, 3.12.0, 3.13.0, 3.14.0 between 30 Jun and 5 Aug 2026, three of them in the three days around the Django 6.1 release, so an unpinned install moves minor versions underneath a tenancy layer that owns query scoping. |
...
A custom manager does not close that leak either. Django reads a related
object through the model's base manager, which is unfiltered by design.
`select_related`, raw SQL, and a database cascade reach the row through no
manager at all. The recipe is smaller than the package, and it is not safer.
`data-lifecycle-and-privacy.md` owns the fan-out that closes it.

| Concern | Choice and version | Disposition and review notes |
|---|---|---|
| Retention scheduler | `django-celery-beat==2.9.0` | **Recommend as the retention runner.** BSD; Production/Stable; declares Django 5.2 and 6.0. Database-backed schedule with an admin surface and a recorded last-run time, which is what makes a retention policy verifiable. A systemd timer is an equally acceptable runner where Celery is not already deployed. That admin surface is also a task-execution control plane. A `PeriodicTask` row holds the task name and the arguments, so change permission on that model runs any task the worker registers, with arguments the editor chooses. Restrict the `django_celery_beat` model permissions to an administrator role, and write a finding where an ordinary staff role holds them. |
...
| Non-production masking (PostgreSQL) | `postgresql_anonymizer`, 3.x line | **Conditional, and vetted at the database layer rather than through the Python gate.** A PostgreSQL extension providing static masking, dynamic masking, and anonymized dumps with referential-integrity-preserving pseudonymization, which is the mechanism the lower-environment pipeline needs. Confirm the installed version against the project's own security announcements at adoption, and run a current 3.x release. Dynamic masking applies to the roles the extension is told to mask, and a table owner or a superuser reads cleartext through it. A Django application that connects as the owner of its tables therefore reads the unmasked column while the environment is believed masked. Give the application a non-owner role in every masked environment, and assert in CI that a read on the application role returns a masked value. |
...
| Candidate | Disposition and safer direction |
|---|---|
| `django-safedelete==1.4.1` | **Existing-install audit only.** BSD-3-Clause but Beta status, last repository activity Mar 2025, no Django version classifiers, and cascade soft-delete raises against `PROTECT` relations. Where it is present, verify Django compatibility and audit the cascade behavior; do not newly adopt it for a 6.0/5.2 baseline. |
...
| gRPC server and client runtime | `grpcio==1.83.0` (23 Jul 2026) | **Recommend, pinned, and upgrade on cadence rather than on scanner output.** Apache-2.0; Python >=3.10; Production/Stable; published by the gRPC authors on a roughly monthly cadence tracking gRPC Core. There is no built-in alternative — serving gRPC is this dependency — so the gate is about the defaults and the floor. It authenticates nothing until an interceptor is installed, and it sets no send-size or concurrency limit. The floor is the awkward part and is worth stating plainly: the two 2024 C-core advisories that reach Python, HPACK-table poisoning in CVE-2024-7246 and zero-copy data corruption in CVE-2024-11407, appear in OSV only as git ranges against the grpc repository with no PyPI-ecosystem entry, so `pip-audit` raises neither against an old wheel, and OSV's PyPI coverage for this distribution stops at the 2023 advisories that 1.56.2 already cleared. Currency is therefore the control rather than a scanner result: pin exactly and move with the release cadence. 1.82.0 was yanked on 7 Jul 2026 for declaring a bad protobuf dependency range and replaced by 1.82.1 the next day, which is the kind of thing an exact pin plus a lockfile refresh catches and a floating range does not. |
| Proto compiler and generated stubs | `grpcio-tools==1.83.0` (23 Jul 2026) | **Recommend as a build-time dependency; keep it out of the production requirements file.** Apache-2.0; Python >=3.10; released in lockstep with grpcio and requiring `grpcio>=1.83.0` and `protobuf>=7.35.1,<8.0.0`. It generates the `_pb2` and `_pb2_grpc` modules and nothing at runtime needs it, which is the same production-requirements rule `django-extensions` carries and for the same reason. One exception is real: django-socio-grpc depends on it directly, so adopting that package puts a compiler in the production image. Regenerate the stubs whenever the runtime moves rather than carrying older generated code — gencode must never be paired with a runtime older than the toolchain that produced it, and because grpcio-tools bundles its own protobuf, generated code can require a newer protobuf runtime than grpcio-tools' own dependency floor declares. |
| Protobuf runtime | `protobuf==7.35.1` (11 Jun 2026) | **Recommend, pinned; on an older line require `>=6.33.5`, or `>=5.29.6` on 5.x.** 3-Clause BSD; Python >=3.10; published by Google. Two recursion denial-of-service advisories set those floors and both are in the pure-Python decoding path: CVE-2025-4565, fixed in 4.25.8, 5.29.5, and 6.31.1, and CVE-2026-0994, fixed in 5.29.6 and 6.33.5 — so 6.33.5 is the effective 6.x floor and the current 7.x line is outside the affected range of both. It arrives transitively with grpcio-tools and every grpcio sibling, all of which require `protobuf>=7.35.1,<8.0.0` at this version, so an install that pins grpcio is already on the 7.x line whether or not it says so. Pin it anyway: the parsing rules that survive a current runtime — `Any` type-URL allow-listing and unknown-field passthrough — are behavior rather than version, and they are in `a08-integrity-and-deserialization.md`, "Insecure deserialization". |
...
| `django-socio-grpc==0.25.0` (24 Sep 2025) | **Existing-install audit only; for new work the defensible default is hand-rolled grpcio servicers alongside Django.** Apache-2.0, Python >=3.10 with classifiers through 3.14, and no advisory in OSV. Four facts decide it. It is still pre-1.0 after five years, and its own settings module warns that 1.0 will change default filter and pagination behavior. It declares no Django classifier at all — its only floor is `django>=4.2` with no upper bound and `djangorestframework` with no floor whatsoever — so support for 5.2, 6.0, and 6.1 is undeclared rather than stated, and has to be verified in CI. There has been no release since 24 Sep 2025. And its defaults are open in the direction that matters: `DEFAULT_AUTHENTICATION_CLASSES` and `DEFAULT_PERMISSION_CLASSES` are empty lists, `SERVER_INTERCEPTORS` and `SERVER_OPTIONS` are `None`, and `REQUIRE_CLIENT_AUTH` is `False`, read from the 0.25.0 distribution on 9 Aug 2026. What it buys is DRF-shaped services and proto generation from serializers; what it costs is a pre-1.0 single-vendor dependency sitting in the authentication path of a second server. Where it is already deployed, audit it against `graphql-and-alternative-api-surfaces.md`, "gRPC: nothing from the DRF request cycle applies". The exit is cheap because the DRF business logic is portable: hand-rolled servicers reusing the same serializers. |
...
The security release is `3.17.2` (5 Aug 2026) and not `3.18.0`. `3.18.0` is
later and carries the same fixes. `3.17.2` stopped `AdminRenderer` from a
disclosure of GET-protected data through a validation-error response. It also
made `request.data` parsing honor Django's `DATA_UPLOAD_MAX_MEMORY_SIZE`
rather than read past it. `3.17.2` is therefore the minimum safe version, and
a project that cannot take `3.18.0` should still be on it.
...
| Celery, and message brokers generally | **Out of scope here; not tiered.** Celery is infrastructure a project already runs, not a security control being selected, so it does not take a disposition in this index. Audit its configuration against A08 — `accept_content`, the result backend, and who can reach the broker — rather than treating its presence as a package decision. `django-celery-beat` is tiered above for the narrower job of running retention. On the 6.0 line the built-in-first question now has to be asked, and the answer is narrower than it looks: `django.tasks` standardizes the task *interface*, but Django 6.0.7 ships only the immediate and dummy backends and neither defers or schedules, so the built-in is the first option for declaring a task and is not yet a replacement for a broker, a worker, or `django-celery-beat`. A durable backend is still third-party on that line. Audit it against A08, "Django's built-in tasks framework". |
...
| Encryption primitives | `cryptography==50.0.0`; minimum safe is the 48.x line (advisories checked 10 Aug 2026) | **Recommend as the base, not as a field library; require the 48.x line or later.** PyCA-maintained, dual Apache-2.0/BSD, healthy cadence; also listed under "Data layer and database" for the storage side. The floor is new and it is not a formality: three 2026 advisories are fixed in 48.x — CVE-2026-39892, a buffer overflow reached by passing a non-contiguous buffer; CVE-2026-34073, name constraints not applied when a certificate carries a wildcard DNS SAN, which is a verification failure rather than a crash; and CVE-2026-26007, a private-key leak on the binary elliptic curves, whose remediation also deprecates the SECT curves, so a project still using one has a migration in front of it rather than only an upgrade. An install below 48.x is a finding on its own weight even though the recommended pin is well above it. `Fernet` is AES-128-CBC plus HMAC-SHA256 and embeds the encryption time in the token as plaintext; `MultiFernet` supplies key versioning and `rotate()`; `AESGCM` and `ChaCha20Poly1305` supply single-pass AEAD where the Fernet timestamp or the CBC construction is unwanted. |
