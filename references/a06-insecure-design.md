...
Keep the request rate limit as well. See `agent-and-llm-interfaces.md`, "Cost
and concurrency limits, not only request rate".
...
- **A page-size query parameter without `max_page_size` removes the ceiling
  ...
  accepts a client `limit` immediately, with `max_limit = None` behind it.
...
and the database binds every other caller. A serializer `validate_` method is
often the natural place to give the client a good error. Write it in addition
to the constraint, and not in place of it. Its job is the message, and the job
of the constraint is the guarantee.
...
Resolve every monetary value from the server's own row, in `perform_create()`
or the service layer. A field that is only validated is still a field the
client chose. Give the quantity its `min_value` and `max_value` on the
serializer field, in the same edit. A negative or very large quantity
multiplied by a correct price is the same manipulation through arithmetic.
...
- **Which are compensating rather than reversing?** A refund is a second,
  ...
  dispute. It also cannot bound a second refund against the same capture.
...
the same edit. It is the only key that a later reconciliation can join on.
...
- [ ] The inventory covers management commands, Celery tasks and beat
      ...
      `bulk_update()`, and `loaddata`. It covers more than viewsets and views.
...
that constraint it silently creates duplicates under concurrent submits. That
duplicate is the failure this flow exists to prevent.
...
- `DATA_UPLOAD_MAX_NUMBER_FIELDS` and `DATA_UPLOAD_MAX_NUMBER_FILES` cap
  multipart complexity. Do not raise either one without a reason.
...
- [ ] Every caller-controlled value that multiplies work has a ceiling that
      the server enforces. Those values are the page size, the depth, the
      nesting, the date range, and the batch or expansion factor. The ceiling
      produces a rejection, and not an out-of-memory kill.
